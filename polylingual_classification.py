...
#TODO: single-fragment
#TODO: check RCV1/2: 101 categories and English
...
if __name__=='__main__':
    ...
    result_id = dataset_name+'_'+op.mode+op.learner+('_optimC' if op.optimc else '')
    ...
    def get_learner(calibrate=False, z_space=False, define_params=False):
        c_range = [1e4, 1e3, 1e2, 1e1, 1, 1e-1]

        if op.learner == 'svm':
            learner = SVC(kernel='linear', probability=calibrate)
    ...
    learner, params = get_learner(define_params=op.optimc)
    final_learner, z_params = get_learner(calibrate=False, z_space=True, define_params=op.optimc)
    ...
    if op.mode == 'class':
        ...
        classifier = ClassEmbeddingPolylingualClassifier(auxiliar_learner=get_learner(calibrate=True),
                                                         ...
                                                         parameters=None, z_parameters=z_params) #optimize only for z_params
    elif op.mode == 'naive':
        print('Learning Naive Poly-lingual Classifier')
        classifier = NaivePolylingualClassifier(base_learner=learner, parameters=params)
    elif op.mode == 'juxta':
        print('Learning Juxtaposed Poly-lingual Classifier')
        classifier = JuxtaposedPolylingualClassifier(base_learner=learner, parameters=params)
        ...
        classifier = ClassJuxtaEmbeddingPolylingualClassifier(auxiliar_learner=get_learner(calibrate=True),
                                                              ...
                                                              alpha=0.5,
                                                              c_parameters=params, y_parameters=params)
    elif op.mode == 'lri':
        ...
        classifier = LRIPolylingualClassifier(base_learner=learner, parameters=params)
    elif op.mode == 'lri-half':
        assert op.learner != 'nb', 'nb operates only on positive matrices'
        print('Learning Lightweight Random Indexing Poly-lingual Classifier')
        classifier = LRIPolylingualClassifier(base_learner=learner, parameters=params, reduction=0.5)
    elif op.mode == 'dci-lin':
        assert op.learner!='nb', 'nb operates only on positive matrices'
        print('Learning Distributional Correspondence Indexing with Linear Poly-lingual Classifier')
        classifier = DCIPolylingualClassifier(base_learner=learner, dcf='linear', z_parameters=z_params)
    elif op.mode == 'dci-pmi':
        assert op.learner != 'nb', 'nb operates only on positive matrices'
        print('Learning Distributional Correspondence Indexing with PMI Poly-lingual Classifier')
        classifier = DCIPolylingualClassifier(base_learner=learner, dcf='pmi', z_parameters=z_params)
    elif op.mode == 'clesa':
        ...
        classifier = CLESAPolylingualClassifier(base_learner=learner, lW=lW, z_parameters=z_params)
    elif op.mode == 'upper':
        ...
        classifier = NaivePolylingualClassifier(base_learner=learner, parameters=params) #this is just to match the multilingual dataset format (despite there are only English documents)
        ...
        classifier = ClassEmbeddingPolylingualClassifier(auxiliar_learner=get_learner(calibrate=True),
                                                         final_learner=final_learner,
                                                         parameters=None, z_parameters=z_params)
    ...
    for lang in data.langs():
        ...
        results.add_row(result_id, op.mode, op.learner, op.optimc, dataset_name, classifier.time, lang, macrof1, microf1, notes=op.note)
