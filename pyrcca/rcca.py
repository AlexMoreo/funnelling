...
class _CCABase(object):
    ...
    def train(self, data):
        ...
        self.train_time = time() - tinit
        return self

    #esto lo he implementado yo...
    def project(self, vdata):
        vdata = [np.nan_to_num(_zscore(d)) for d in vdata]
        if not hasattr(self, 'ws'):
            raise NameError('Algorithm has not been trained.')
        return _listdot([d.T for d in vdata], self.ws)
...
def kcca(data, reg=0., numCC=None, kernelcca=True, ktype='linear', gausigma=1.0, degree=2):
    ...
    if kernelcca:
        kernel = Parallel(n_jobs=-1)(delayed(_make_kernel)(d, True, ktype, gausigma, degree) for d in data)
        # kernel = [_make_kernel(d, ktype=ktype, gausigma=gausigma, degree=degree, normalize=True) for d in data]
    else:
    ...
    crosscovs = Parallel(n_jobs=-1)(delayed(np.dot)(ki, kj.T) for ki in kernel for kj in kernel)
    ...
    for i in range(nDs):
        RH[sum(nFs[:i]) : sum(nFs[:i+1]),
           sum(nFs[:i]) : sum(nFs[:i+1])] = (crosscovs[i * (nDs + 1)] + reg * np.eye(nFs[i]))
        for j in range(nDs):
            if i != j:
