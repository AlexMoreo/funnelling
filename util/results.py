...
class PolylingualClassificationResults:
    def __init__(self, file, autoflush=True, verbose=False):
        self.file = file
        self.columns = ['id', 'method', 'dataset', 'time', 'lang', 'macrof1', 'microf1', 'macrok', 'microk', 'notes']
    ...
    def add_row(self, id, method, dataset, time, lang, macrof1, microf1, macrok, microk, notes=''):
        s = pd.Series([id, method, dataset, time, lang, macrof1, microf1, macrok, microk, notes], index=self.columns)
